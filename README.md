# MAF ready to validate, validated on A100, TF2.8.0 and 2.9.0
Original repo: https://github.com/peteryuX/arcface-tf2
...
Train data:
```bash
gdown --fuzzy 'https://drive.google.com/file/d/1X202mvYe5tiXFhOx82z4rPiPogXD435i/view'
unzip ms1m_align_112.zip -d data/ms1m_align_112
python data/convert_train_binary_tfrecord.py --dataset_path="/path/to/data/ms1m_align_112/imgs" --output_path="./data/ms1m_bin.tfrecord"
```
Test data:
```bash
gdown --fuzzy 'https://drive.google.com/file/d/1WO5Meh_yAau00Gm2Rz2Pc0SRldLQYigT/view?usp=sharing'
unzip lfw_align_112.zip -d test_data/lfw_align_112
```

## Training
Update dataset path or other settings of model in ./configs/*.yaml for training and testing, which like below.
```
train_dataset: './data/ms1m_bin.tfrecord' # or './data/ms1m.tfrecord'
test_dataset: '/your/path/to/test_data'
```
Run training
```bash
python train.py --mode="eager_tf" --cfg_path="./configs/arc_res50.yaml" 2>&1 | tee arcface_tf280_a100.log
```
